	...
	BEGIN_MSG_MAP( CAxHostWindow )
...
#endif
		{
			...
			if (dwHitResult == HITRESULT_HIT)
			{
				...
				//x MESSAGE_HANDLER(WM_SETCURSOR, OnWindowlessMouseMessage) 		//+++ 旧版にあわせて追加...やっぱり削除.
		...
		//+++ if(!m_bWindowless && m_bMessageReflect)			//+++ 旧版にあわせる
...
RECT rect;
GetClientRect(&rect);
::FillRect( (HDC) wParam, &rect, (HBRUSH) LongToPtr(COLOR_ACTIVECAPTION) );
#endif
		...
		*ppUnk = NULL;
		...
		if (::IsWindow(hWnd))
		{
			...
			if (SUCCEEDED(hr) && bWasHTML && *ppUnk != NULL)
			{
			  ...
			  #if 0 //*+++ donut(系)ではこの処理があるとabout:blankがらみのバグが発生する.
				//	//* ※ BeforeNavigate等の呼び出しタイミングが変わり処理が破綻.
				//	else
				//	{
...
// IOleInPlaceSite
	...
	STDMETHOD(OnPosRectChange)(LPCRECT lprcPosRect)
	{
		...
		// Do the actual move.
		//MoveWindow( &rect);	//\\+ window.resizeTo などのときに呼ばれる？　無視する処理
...
#pragma warning(pop)
	...
	STDMETHODIMP ShowMessage(HWND hwnd, LPOLESTR lpstrText, LPOLESTR lpstrCaption, DWORD dwType, LPOLESTR lpstrHelpFile, DWORD dwHelpContext, LRESULT *plResult)
	{
		...
		if (hWndMDIActive == GetParent()) {
			...
			if (nMode)
				MtlSetStatusText(hWndStatus, lpstrText);
			else
				MtlSetWindowText(hWndStatus, lpstrText);
		}
	...
	case WM_CREATE:
		{
		  ...
		  #else
			TCHAR*	spName = (TCHAR *)_alloca((nLen + 1) * sizeof(TCHAR));
		  #endif
			if(!spName)
			{
				return -1;
			...
			USES_CONVERSION;
			CComPtr<IUnknown> spUnk;
			HRESULT hRet = AtlAxCreateControlLic(T2COLE(spName), hWnd, spStream, &spUnk, NULL);
		  #endif
			if(FAILED(hRet))
			{
#ifdef _DEBUG
...
static LRESULT CALLBACK AtlAxWindowProc2(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		// create control from a PROGID in the title
			...
			USES_CONVERSION;
			CComPtr<IUnknown> spUnk;
			hRet = AtlAxCreateControlLic(T2COLE(spName), hWnd, spStream, &spUnk, bstrLicKey);
		  #endif
			if(FAILED(hRet))
			{
#ifdef _DEBUG

/**
 *	@file	AtlhostEx_for_atl.h
 *	@brief	DonutP 5β3 のソースにあった AtlhostEx.h を修正したもの.
 *	@note
 *		atl3.?でコンパイルするために DonutP 5β3 のソースにあった AtlhostEx.h を修正したもの.
 *		Platform SDK(2003 r2)に入っていたatlhost.hでの変更点(バグとり,Win64対応)も反映している.
 */
...
#endif
		public IDispatchImpl<IAxWinAmbientDispatchEx_UNDONUT, &IID_IAxWinAmbientDispatchEx_UNDONUT, &LIBID_ATLLib>,
...
public:
		...
		EXCEPINFO* pexcepinfo, UINT* puArgErr)
	{
		if (dispidMember == DISPID_AMBIENT_DLCONTROL) {
			...
			CComVariant var((long)m_dwDLControlFlags, VT_I4);
...
// ctor/dtor
	...
	STDMETHOD(SetIeMenuNoCstm)(int nStatus)
	{
		m_nIeMenuNoCstm = nStatus;
		return S_OK;
	}

	CAxHostWindow()
	{
		UINT nValue = ::GetPrivateProfileInt(_T("ETC"), _T("IeMenuNoCstm"), FALSE, g_szIniFileName/*GetPath()*/);
		m_nIeMenuNoCstm = nValue;
	...
	BEGIN_COM_MAP(CAxHostWindow)
		COM_INTERFACE_ENTRY2(IDispatch, IAxWinAmbientDispatchEx_UNDONUT)
		...
		COM_INTERFACE_ENTRY(IAxWinAmbientDispatchEx_UNDONUT)
#ifndef _ATL_NO_DOCHOSTUIHANDLER
...
//		MESSAGE_HANDLER(WM_MOUSEACTIVATE, OnMouseActivate)				//+++ 旧版にあわせる
		MESSAGE_HANDLER(WM_SETFOCUS, OnSetFocus)
		MESSAGE_HANDLER(WM_KILLFOCUS, OnKillFocus)
		//+++ if (m_bWindowless)
		if (m_bWindowless && uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) 	//+++ 旧版にあわせる
		{
...
// IDocHostUIHandler
	HRESULT ShowContextMenuEx(POINT* pptPosition)
	{
		...
		::SendMessage(GetTopLevelWindow(), WM_COMMAND, iSelection, NULL);
		return S_OK;
	}
	...
	void CstmContextMenu(HMENU hMenu, DWORD dwID, CSimpleMap<DWORD, DWORD> &mapCmd, CSimpleArray<HMENU> &aryDestroyMenu)
	{
		...
		CString strPath = _GetFilePath(_T("Menu.ini"));
		DWORD dwCount=0;
		CString strSection; 	strSection.Format(_T("Type%d"), dwID);
...
class _DialogSplitHelper_UNDONUT
{
public:
...
ATLINLINE ATLAPI_(INT_PTR) AtlAxDialogBoxW(HINSTANCE hInstance, LPCWSTR lpTemplateName, HWND hWndParent, DLGPROC lpDialogProc, LPARAM dwInitParam)
{
	...
	if (hDlg)
	{
		...
		lpDialogTemplate = _DialogSplitHelper_UNDONUT::SplitDialogTemplate(pDlg, pInitData);

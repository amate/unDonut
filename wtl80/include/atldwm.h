...
#else	//#endif // (_WIN32_WINNT < 0x0600)		//+++
	#define	DwmApi_Wrap							//+++
...
public:
 ...
 // Operations
	BOOL DwmIsCompositionEnabled() const
	{
		...
		BOOL 	bRes = FALSE;
		return SUCCEEDED(DwmApi_Wrap::DwmIsCompositionEnabled(&bRes)) && bRes;
	}

	BOOL DwmEnableComposition(UINT fEnable)
	{
		if (!IsDwmSupported()) return FALSE;
		return SUCCEEDED(DwmApi_Wrap::DwmEnableComposition(fEnable));
	}

	BOOL DwmEnableMMCSS(BOOL fEnableMMCSS)
	{
		if (!IsDwmSupported()) return FALSE;
		return SUCCEEDED(DwmApi_Wrap::DwmEnableMMCSS(fEnableMMCSS));
	}

	HRESULT DwmGetColorizationColor(DWORD* pcrColorization, BOOL* pfOpaqueBlend)
	{
		if (!IsDwmSupported()) return E_NOTIMPL;
		return DwmApi_Wrap::DwmGetColorizationColor(pcrColorization, pfOpaqueBlend);
	}

	HRESULT DwmFlush()
	{
		if (!IsDwmSupported()) return E_NOTIMPL;
		return DwmApi_Wrap::DwmFlush();
	}
};
...
public:
	HRESULT DwmEnableBlurBehindWindow(const DWM_BLURBEHIND* pBB)
	{
		...
		return DwmApi_Wrap::DwmEnableBlurBehindWindow(pT->m_hWnd, pBB);
	}
	...
	HRESULT DwmExtendFrameIntoClientArea(const MARGINS* pMargins)
	{
		...
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return DwmApi_Wrap::DwmExtendFrameIntoClientArea(pT->m_hWnd, pMargins);
	}
	...
	HRESULT DwmGetCompositionTimingInfo(DWM_TIMING_INFO* pTimingInfo)
	{
		...
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return DwmApi_Wrap::DwmGetCompositionTimingInfo(pT->m_hWnd, pTimingInfo);
	}
	...
	HRESULT DwmGetWindowAttribute(DWORD dwAttribute, PVOID pvAttribute, DWORD cbAttribute)
	{
		...
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return DwmApi_Wrap::DwmGetWindowAttribute(pT->m_hWnd, dwAttribute, pvAttribute, cbAttribute);
	}
	...
	HRESULT DwmModifyPreviousDxFrameDuration(INT cRefreshes, BOOL fRelative)
	{
		...
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return DwmApi_Wrap::DwmModifyPreviousDxFrameDuration(pT->m_hWnd, cRefreshes, fRelative);
	}
	...
	HRESULT DwmSetDxFrameDuration(INT cRefreshes)
	{
		...
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return DwmApi_Wrap::DwmSetDxFrameDuration(pT->m_hWnd, cRefreshes);
	}
	...
	HRESULT DwmSetPresentParameters(DWM_PRESENT_PARAMETERS* pPresentParams)
	{
		...
		T* 	pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return DwmApi_Wrap::DwmSetPresentParameters(pT->m_hWnd, pPresentParams);
	}
	...
	HRESULT DwmSetWindowAttribute(DWORD dwAttribute, LPCVOID pvAttribute, DWORD cbAttribute)
	{
		...
		T* 	pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return DwmApi_Wrap::DwmSetWindowAttribute(pT->m_hWnd, dwAttribute, pvAttribute, cbAttribute);
	}
	...
	HRESULT DwmAttachMilContent()
	{
		...
		T* 	pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return DwmApi_Wrap::DwmAttachMilContent(pT->m_hWnd);
	}
	...
	HRESULT DwmDetachMilContent()
	{
		...
		T* 	pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return DwmApi_Wrap::DwmDetachMilContent(pT->m_hWnd);
	}
};
...
public:
 ...
 // Operations
	...
	HRESULT Register(HWND hwndDestination, HWND hwndSource)
	{
		...
		if (!IsDwmSupported()) return E_NOTIMPL;
		return DwmApi_Wrap::DwmRegisterThumbnail(hwndDestination, hwndSource, &m_hThumbnail);
	}
	...
	HRESULT Unregister()
	{
		...
		HRESULT Hr = DwmApi_Wrap::DwmUnregisterThumbnail(m_hThumbnail);
	...
	HRESULT UpdateProperties(const DWM_THUMBNAIL_PROPERTIES* ptnProperties)
	{
		if (!IsDwmSupported()) return E_NOTIMPL;
		ATLASSERT(m_hThumbnail != NULL);
		return DwmApi_Wrap::DwmUpdateThumbnailProperties(m_hThumbnail, ptnProperties);
	}
 ...
 // Attributes
	HRESULT QuerySourceSize(PSIZE pSize)
	{
		if (!IsDwmSupported()) return E_NOTIMPL;
		ATLASSERT(m_hThumbnail != NULL);
		return DwmApi_Wrap::DwmQueryThumbnailSourceSize(m_hThumbnail, pSize);
	}
};
...
public:
 ...
 // Implementation
	...
	LRESULT DefWindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		T* 		pT 	 = static_cast<T*>(this);
		LRESULT lRes = 0;
		if ( DwmApi_Wrap::DwmDefWindowProc(pT->m_hWnd, uMsg, wParam, lParam, &lRes) ) return lRes;

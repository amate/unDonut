...
#endif
#if (_WIN32_WINNT < 0x0600)			//+++ uxtheme.dll の扱いの辻褄あわせ. CThemeが使われてもハングしないように.
									//+++ .vcprojの追加ライブラリにdelayimp.lib, 遅延ロードに uxtheme.dllを設定しても効かないようなので、これを用意...
...
public:
 ...
 // Theme support helper
	static bool IsThemingSupported()
	{
		...
		return m_nIsThemingSupported > 0;	//(m_nIsThemingSupported == 1);
	}
 ...
 // Operations and theme properties
	...
	HRESULT CloseThemeData()
	{
	  ...
	  #if 1 //+++
		if (m_nIsThemingSupported <= 0)
			m_hTheme = NULL;
	  #endif
		if (m_hTheme != NULL)
		{
			hRet = UxTheme_Wrap::CloseThemeData(m_hTheme);
			if (SUCCEEDED(hRet))
				m_hTheme = NULL;
		}
		return hRet;
	...
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::DrawThemeBackgroundEx(m_hTheme, hDC, nPartID, nStateID, pRect, pOptions);
	}
	...
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::DrawThemeText(m_hTheme, hDC, nPartID, nStateID, pszText, nCharCount, dwTextFlags, dwTextFlags2, pRect);
	}

	HRESULT GetThemeBackgroundContentRect(HDC hDC, int nPartID, int nStateID,  LPCRECT pBoundingRect, LPRECT pContentRect) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeBackgroundContentRect(m_hTheme, hDC, nPartID, nStateID,  pBoundingRect, pContentRect);
	}

	HRESULT GetThemeBackgroundExtent(HDC hDC, int nPartID, int nStateID, LPCRECT pContentRect, LPRECT pExtentRect) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeBackgroundExtent(m_hTheme, hDC, nPartID, nStateID, pContentRect, pExtentRect);
	}

	HRESULT GetThemePartSize(HDC hDC, int nPartID, int nStateID, LPRECT pRect, enum THEMESIZE eSize, LPSIZE pSize) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemePartSize(m_hTheme, hDC, nPartID, nStateID, pRect, eSize, pSize);
	}
	...
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeTextExtent(m_hTheme, hDC, nPartID, nStateID, pszText, nCharCount, dwTextFlags, pBoundingRect, pExtentRect);
	}
	...
	HRESULT GetThemeTextMetrics(HDC hDC, int nPartID, int nStateID, PTEXTMETRICW pTextMetric) const
	{
		...
		return UxTheme_Wrap::GetThemeTextMetrics(m_hTheme, hDC, nPartID, nStateID, pTextMetric);
	  #else // !_WTL_NEW_UXTHEME
		// Note: The cast to PTEXTMETRIC is because uxtheme.h incorrectly uses it instead of PTEXTMETRICW
		return UxTheme_Wrap::GetThemeTextMetrics(m_hTheme, hDC, nPartID, nStateID, (PTEXTMETRIC)pTextMetric);
	  #endif // !_WTL_NEW_UXTHEME
	}
	...
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeBackgroundRegion(m_hTheme, hDC, nPartID, nStateID, pRect, pRegion);
	}
	...
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::HitTestThemeBackground(m_hTheme, hDC, nPartID, nStateID, dwOptions, pRect, hrgn, ptTest, pwHitTestCode);
	}

	HRESULT DrawThemeEdge(HDC hDC, int nPartID, int nStateID, LPCRECT pDestRect, UINT uEdge, UINT uFlags, LPRECT pContentRect = NULL)
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::DrawThemeEdge(m_hTheme, hDC, nPartID, nStateID, pDestRect, uEdge, uFlags, pContentRect);
	}

	HRESULT DrawThemeIcon(HDC hDC, int nPartID, int nStateID, LPCRECT pRect, HIMAGELIST himl, int nImageIndex)
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::DrawThemeIcon(m_hTheme, hDC, nPartID, nStateID, pRect, himl, nImageIndex);
	}

	BOOL IsThemePartDefined(int nPartID, int nStateID) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::IsThemePartDefined(m_hTheme, nPartID, nStateID);
	}

	BOOL IsThemeBackgroundPartiallyTransparent(int nPartID, int nStateID) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::IsThemeBackgroundPartiallyTransparent(m_hTheme, nPartID, nStateID);
	}

	HRESULT GetThemeColor(int nPartID, int nStateID, int nPropID, COLORREF* pColor) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeColor(m_hTheme, nPartID, nStateID, nPropID, pColor);
	}

	HRESULT GetThemeMetric(HDC hDC, int nPartID, int nStateID, int nPropID, int* pnVal) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeMetric(m_hTheme, hDC, nPartID, nStateID, nPropID, pnVal);
	}

	HRESULT GetThemeString(int nPartID, int nStateID, int nPropID, LPWSTR pszBuff, int cchMaxBuffChars) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeString(m_hTheme, nPartID, nStateID, nPropID, pszBuff, cchMaxBuffChars);
	}

	HRESULT GetThemeBool(int nPartID, int nStateID, int nPropID, BOOL* pfVal) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeBool(m_hTheme, nPartID, nStateID, nPropID, pfVal);
	}

	HRESULT GetThemeInt(int nPartID, int nStateID, int nPropID, int* pnVal) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeInt(m_hTheme, nPartID, nStateID, nPropID, pnVal);
	}

	HRESULT GetThemeEnumValue(int nPartID, int nStateID, int nPropID, int* pnVal) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeEnumValue(m_hTheme, nPartID, nStateID, nPropID, pnVal);
	}

	HRESULT GetThemePosition(int nPartID, int nStateID, int nPropID, LPPOINT pPoint) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemePosition(m_hTheme, nPartID, nStateID, nPropID, pPoint);
	}
	...
	HRESULT GetThemeFont(int nPartID, HDC hDC, int nStateID, int nPropID, LOGFONTW* pFont) const
	{
		ATLASSERT(m_hTheme != NULL);
	  #ifdef _WTL_NEW_UXTHEME
		return UxTheme_Wrap::GetThemeFont(m_hTheme, hDC, nPartID, nStateID, nPropID, pFont);
	  #else // !_WTL_NEW_UXTHEME
		// Note: The cast to LOGFONT* is because uxtheme.h incorrectly uses it instead of LOGFONTW*
		return UxTheme_Wrap::GetThemeFont(m_hTheme, hDC, nPartID, nStateID, nPropID, (LOGFONT*)pFont);
	  #endif // !_WTL_NEW_UXTHEME
	}
	...
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeRect(m_hTheme, nPartID, nStateID, nPropID, pRect);
	}

	HRESULT GetThemeMargins(HDC hDC, int nPartID, int nStateID, int nPropID, LPRECT pRect, PMARGINS pMargins) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeMargins(m_hTheme, hDC, nPartID, nStateID, nPropID, pRect, pMargins);
	}

	HRESULT GetThemeIntList(int nPartID, int nStateID, int nPropID, INTLIST* pIntList) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeIntList(m_hTheme, nPartID, nStateID, nPropID, pIntList);
	}

	HRESULT GetThemePropertyOrigin(int nPartID, int nStateID, int nPropID, enum PROPERTYORIGIN* pOrigin) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemePropertyOrigin(m_hTheme, nPartID, nStateID, nPropID, pOrigin);
	}

	HRESULT GetThemeFilename(int nPartID, int nStateID, int nPropID, LPWSTR pszThemeFileName, int cchMaxBuffChars) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeFilename(m_hTheme, nPartID, nStateID, nPropID, pszThemeFileName, cchMaxBuffChars);
	}

	COLORREF GetThemeSysColor(int nColorID) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeSysColor(m_hTheme, nColorID);
	}

	HBRUSH GetThemeSysColorBrush(int nColorID) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeSysColorBrush(m_hTheme, nColorID);
	}

	int GetThemeSysSize(int nSizeID) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeSysSize(m_hTheme, nSizeID);
	}

	BOOL GetThemeSysBool(int nBoolID) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeSysBool(m_hTheme, nBoolID);
	}
	...
	HRESULT GetThemeSysFont(int nFontID, LOGFONTW* plf) const
	{
		ATLASSERT(m_hTheme != NULL);
	  #ifdef _WTL_NEW_UXTHEME
		return UxTheme_Wrap::GetThemeSysFont(m_hTheme, nFontID, plf);
	  #else // !_WTL_NEW_UXTHEME
		// Note: The cast to LOGFONT* is because uxtheme.h incorrectly uses it instead of LOGFONTW*
		return UxTheme_Wrap::GetThemeSysFont(m_hTheme, nFontID, (LOGFONT*)plf);
	  #endif // !_WTL_NEW_UXTHEME
	}
	...
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeSysString(m_hTheme, nStringID, pszStringBuff, cchMaxStringChars);
	}

	HRESULT GetThemeSysInt(int nIntID, int* pnValue) const
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeSysInt(m_hTheme, nIntID, pnValue);
	}
	...
	HTHEME OpenThemeDataEx(HWND hWnd, LPCWSTR pszClassList, DWORD dwFlags)
	{
		...
		ATLASSERT(m_hTheme == NULL);
		m_hTheme = UxTheme_Wrap::OpenThemeDataEx(hWnd, pszClassList, dwFlags);
		return m_hTheme;
	}
	...
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::DrawThemeTextEx(m_hTheme, hDC, nPartID, nStateID, pszText, cchText, dwTextFlags, lpRect, pOptions);
	}

	HRESULT GetThemeTransitionDuration(int nPartID, int nFromStateID, int nToStateID, int nPropID, DWORD& dwDuration)
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeTransitionDuration(m_hTheme, nPartID, nFromStateID, nToStateID, nPropID, &dwDuration);
	}
 #endif // _WTL_NEW_UXTHEME

 // #if (_WIN32_WINNT >= 0x0600)		//+++ 条件追加
 #if (_WIN32_WINNT >= 0x0600) || defined(UxTheme_Wrap) == 0	//+++ UxTheme_Wrapが#defineでなくnamespaceで作られてたら、こっち
	HRESULT GetThemeBitmap(int nPartID, int nStateID, int nPropID, ULONG uFlags, HBITMAP& hBitmap)
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeBitmap(m_hTheme, nPartID, nStateID, nPropID, uFlags, &hBitmap);
	}

	HRESULT GetThemeStream(int nPartID, int nStateID, int nPropID, VOID** ppvStream, DWORD* pcbStream, HINSTANCE hInstance)
	{
		ATLASSERT(m_hTheme != NULL);
		return UxTheme_Wrap::GetThemeStream(m_hTheme, nPartID, nStateID, nPropID, ppvStream, pcbStream, hInstance);
	}
 #else	//+++ 念のため
	HRESULT GetThemeBitmap(int nPartID, int nStateID, int nPropID, ULONG uFlags, HBITMAP& hBitmap)
	{
		if (m_nIsThemingSupported <= 0 || m_hTheme == 0)
			return S_FALSE;
		HMODULE hThemeDLL = ::LoadLibrary(_T("uxtheme.dll"));
		if (hThemeDLL == NULL)
			return S_FALSE;
		typedef HRESULT (*fn_GetThemeBitmap_t)(HTHEME hTheme, int nPartID, int nStateID, int nPropID, ULONG uFlags, HBITMAP* hBitmap);
		fn_GetThemeBitmap_t	fn_GetThemeBitmap = (fn_GetThemeBitmap_t)::GetProcAddress( hThemeDLL, "GetThemeBitmap" );
		if (fn_GetThemeBitmap == 0)
			return S_FALSE;
		HRESULT hr = fn_GetThemeBitmap(m_hTheme, nPartID, nStateID, nPropID, uFlags, &hBitmap);
		::FreeLibrary(hThemeDLL);
		return hr;
	}
	...
	HRESULT GetThemeStream(int nPartID, int nStateID, int nPropID, VOID** ppvStream, DWORD* pcbStream, HINSTANCE hInstance)
	{
		...
		if (hThemeDLL == NULL)
			return S_FALSE;
		typedef HRESULT (*fn_GetThemeStream_t)(HTHEME hTheme, int nPartID, int nStateID, int nPropID, VOID** ppvStream, DWORD* pcbStream, HINSTANCE hInstance);
		fn_GetThemeStream_t		fn_GetThemeStream = (fn_GetThemeStream_t)::GetProcAddress( hThemeDLL, "GetThemeStream" );
		if (fn_GetThemeStream == 0)
			return S_FALSE;
		HRESULT hr = fn_GetThemeStream(m_hTheme, nPartID, nStateID, nPropID, ppvStream, pcbStream, hInstance);
		::FreeLibrary(hThemeDLL);
		return hr;
	}
...
public:
 ...
 // Operations
	...
	HRESULT SetWindowTheme(LPCWSTR pszSubAppName, LPCWSTR pszSubIDList)
	{
		...
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return UxTheme_Wrap::SetWindowTheme(pT->m_hWnd, pszSubAppName, pszSubIDList);
	}
	...
	HTHEME GetWindowTheme() const
	{
		...
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return UxTheme_Wrap::GetWindowTheme(pT->m_hWnd);
	}
	...
	HRESULT EnableThemeDialogTexture(DWORD dwFlags)
	{
		...
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return UxTheme_Wrap::EnableThemeDialogTexture(pT->m_hWnd, dwFlags);
	}
	...
	BOOL IsThemeDialogTextureEnabled() const
	{
		...
		const T* pT = static_cast<const T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return UxTheme_Wrap::IsThemeDialogTextureEnabled(pT->m_hWnd);
	}
	...
	HRESULT DrawThemeParentBackground(HDC hDC, const RECT* pRect = NULL)
	{
		...
		ATLASSERT(::IsWindow(pT->m_hWnd));
	  #ifdef _WTL_NEW_UXTHEME
		return UxTheme_Wrap::DrawThemeParentBackground(pT->m_hWnd, hDC, pRect);
	  #else
		return UxTheme_Wrap::DrawThemeParentBackground(pT->m_hWnd, hDC, (RECT*)pRect);
	  #endif
	}
	...
	HRESULT SetWindowThemeAttribute(WINDOWTHEMEATTRIBUTETYPE type, PVOID pvAttribute, DWORD cbAttribute)
	{
		...
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return UxTheme_Wrap::SetWindowThemeAttribute(pT->m_hWnd, type, pvAttribute, cbAttribute);
	}
	...
	HRESULT SetWindowThemeNonClientAttributes(DWORD dwAttributes, DWORD dwMask)
	{
		...
		return UxTheme_Wrap::SetWindowThemeAttribute(pT->m_hWnd, WTA_NONCLIENT, (PVOID)&opt, sizeof(opt));
	}
	...
	HRESULT DrawThemeParentBackgroundEx(HDC hDC, DWORD dwFlags, const RECT* lpRect = NULL)
	{
		...
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(pT->m_hWnd));
		return UxTheme_Wrap::DrawThemeParentBackgroundEx(pT->m_hWnd, hDC, dwFlags, lpRect);
	}
 #endif // _WTL_NEW_UXTHEME
...
public:
	...
	CBufferedPaintBase()
	{
		if (IsBufferedPaintSupported())
			ATLVERIFY(SUCCEEDED(UxTheme_Wrap::BufferedPaintInit()));
	}

	~CBufferedPaintBase()
	{
		if (IsBufferedPaintSupported())
			ATLVERIFY(SUCCEEDED(UxTheme_Wrap::BufferedPaintUnInit()));
	}
	...
	static bool IsBufferedPaintSupported()
	{
		if (m_nIsBufferedPaintSupported == -1)
		{
			...
			if (m_nIsBufferedPaintSupported == -1)
				m_nIsBufferedPaintSupported = RunTimeHelper::IsVista() != 0;	//+++ ? 1 : 0;
...
public:
	...
	HPAINTBUFFER Begin(HDC hdcTarget, const RECT* prcTarget, BP_BUFFERFORMAT dwFormat, BP_PAINTPARAMS* pPaintParams, HDC* phdcPaint)
	{
		...
		m_hPaintBuffer = UxTheme_Wrap::BeginBufferedPaint(hdcTarget, prcTarget, dwFormat, pPaintParams, phdcPaint);
	...
	HRESULT End(BOOL bUpdate = TRUE)
	{
		...
		{
			hRet 	= UxTheme_Wrap::EndBufferedPaint(m_hPaintBuffer, bUpdate);
			m_hPaintBuffer = NULL;
		}
		return hRet;
	}
	...
	{
		ATLASSERT(m_hPaintBuffer != NULL);
		return UxTheme_Wrap::GetBufferedPaintTargetRect(m_hPaintBuffer, pRect);
	}

	HDC GetTargetDC() const
	{
		ATLASSERT(m_hPaintBuffer != NULL);
		return UxTheme_Wrap::GetBufferedPaintTargetDC(m_hPaintBuffer);
	}

	HDC GetPaintDC() const
	{
		ATLASSERT(m_hPaintBuffer != NULL);
		return UxTheme_Wrap::GetBufferedPaintDC(m_hPaintBuffer);
	}

	HRESULT GetBits(RGBQUAD** ppbBuffer, int* pcxRow) const
	{
		ATLASSERT(m_hPaintBuffer != NULL);
		return UxTheme_Wrap::GetBufferedPaintBits(m_hPaintBuffer, ppbBuffer, pcxRow);
	}

	HRESULT Clear(const RECT* pRect = NULL)
	{
		ATLASSERT(m_hPaintBuffer != NULL);
		return UxTheme_Wrap::BufferedPaintClear(m_hPaintBuffer, pRect);
	}

	HRESULT SetAlpha(BYTE alpha, const RECT* pRect = NULL)
	{
		ATLASSERT(m_hPaintBuffer != NULL);
		return UxTheme_Wrap::BufferedPaintSetAlpha(m_hPaintBuffer, pRect, alpha);
	}

	HRESULT MakeOpaque(const RECT* pRect = NULL)
	{
		ATLASSERT(m_hPaintBuffer != NULL);
		return UxTheme_Wrap::BufferedPaintSetAlpha(m_hPaintBuffer, pRect, 255);
	}
};
...
public:
	...
	HANIMATIONBUFFER Begin(HWND hWnd, HDC hDCTarget, const RECT* pRectTarget, BP_BUFFERFORMAT dwFormat, BP_PAINTPARAMS* pPaintParams, BP_ANIMATIONPARAMS* pAnimationParams, HDC* phdcFrom, HDC* phdcTo)
	{
		...
		m_hAnimationBuffer = UxTheme_Wrap::BeginBufferedAnimation(hWnd, hDCTarget, pRectTarget, dwFormat, pPaintParams, pAnimationParams, phdcFrom, phdcTo);
	...
	HRESULT End(BOOL bUpdate = TRUE)
	{
		...
		{
			hRet = UxTheme_Wrap::EndBufferedAnimation(m_hAnimationBuffer, bUpdate);
	...
	{
		return (UxTheme_Wrap::BufferedPaintRenderAnimation(hWnd, hDC) != FALSE);
	}
};

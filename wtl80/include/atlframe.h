...
public:
 ...
 // Methods
			...
			DWORD dwStyle = ATL_SIMPLE_TOOLBAR_STYLE, UINT nID = ATL_IDW_TOOLBAR)
	{
		...
		int 	nItems = pData->wItemCount + (bInitialSeparator != 0/*+++ ? 1 : 0*/);
		...
		int nBmp   = 0;
		for (int i = 0, j = bInitialSeparator != 0/*+++ ? 1 : 0*/; i < pData->wItemCount; i++, j++)
		{
			if (pItems[i] != 0)
			{
	...
	LRESULT OnDestroy(UINT, WPARAM, LPARAM, BOOL& bHandled)
	{
		if ((GetStyle() & (WS_CHILD | WS_POPUP)) == 0) {
			...
			::PostQuitMessage(1);
		  #else	//+++
			::PostQuitMessage(0);		//+++ アプリの正常終了時は0を返してほしぃ...ので、強制修正.
		  #endif
		}
...
public:
 ...
 // Operations
	...
	{
	  #if 1	//+++ ASSERTだとDestroy後の通過で、エラー終了するので、チェックで弾くタイプに変更.
		if (::IsWindow(m_hWndMDIClient) == 0) {
...
public:
 ...
 // methods for setting UI element state
	...
	{
		return UISetCheck(nID, bCheck != 0/*+++ ? 1 : 0*/, bForceUpdate);
	}
...
// methods for complete state set/get
 ...
 // internal element specific methods
	...
	static void UIUpdateToolBarElement(int nID, _AtlUpdateUIData* pUIData, HWND hWndToolBar)
	{
		...
		::SendMessage(hWndToolBar, TB_ENABLEBUTTON	, nID, (LPARAM)(pUIData->m_wState & UPDUI_DISABLED) == 0/*+++ ? FALSE:  TRUE*/);
		::SendMessage(hWndToolBar, TB_CHECKBUTTON	, nID, (LPARAM)(pUIData->m_wState & UPDUI_CHECKED ) != 0/*+++ ? TRUE : FALSE*/);
		::SendMessage(hWndToolBar, TB_INDETERMINATE	, nID, (LPARAM)(pUIData->m_wState & UPDUI_CHECKED2) != 0/*+++ ? TRUE : FALSE*/);
		::SendMessage(hWndToolBar, TB_PRESSBUTTON	, nID, (LPARAM)(pUIData->m_wState & UPDUI_RADIO   ) != 0/*+++ ? TRUE : FALSE*/);
	}
